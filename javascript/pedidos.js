$(document).ready(function () {
  ...
  $("#FormCancelaVenda").submit(function () {
    ...
    $.ajax({
      type: "POST",
      url: $("#FormCancelaVenda").attr("action"),
      data: formdata,
  ...
  $("#FormCancelaItemPedido").submit(function () {
    ...
    var formdata =
      $("#FormCancelaItemPedido").serialize() +
      "&pedido_aguarda_venda=" +
      pedido_aguarda_venda;

    $.ajax({
      type: "POST",
      url: $("#FormCancelaItemPedido").attr("action"),
      data: formdata,
      success: function (msg) {
        if (msg == 0) {
          ...
          if (tela_mobile == 1) {
            $.post(
              "menu_pedidos/actions/pega_qtd_itens_pedido.php",
              { id: 1 },
              function (resposta) {
          ...
          $("#resumo-pedido-comanda").html(
            "<br><center>CARREGANDO...</center>"
          );
          $("#resumo-pedido-comanda").load(
            "menu_pedidos/listagem/listagem_itens_pedido.php"
          );
  ...
  $("#FormCancelaAdicionalPedido").submit(function () {
    ...
    $.ajax({
      type: "POST",
      url: $("#FormCancelaAdicionalPedido").attr("action"),
      data: formdata,
      success: function (msg) {
        if (msg == 0) {
  ...
  $("#FormCancelaOpcoesPedido").submit(function () {
    ...
    $.ajax({
      type: "POST",
      url: $("#FormCancelaOpcoesPedido").attr("action"),
      data: formdata,
      success: function (msg) {
        if (msg == 0) {
...
function seleciona_outro_endereco_cliente(id_endereco, id_cliente) {
  ...
  $.post(
    "menu_clientes/actions/recupera_dados_endereco.php",
    { id_endereco: id_endereco, id_cliente: id_cliente },
    function (resposta) {
...
function procura_pedido(busca) {
  $("#reload").html(
    '<div class="col-md-12"><h5><center><br>PROCURANDO...</center></h5></div>'
  );
  $.post(
    "menu_pedidos/listagem/listagem_pedidos_aguardando.php",
    { busca: busca },
    function (resposta) {
...
function mais_item_pedido() {
  ...
  if (tela_mobile == 1) {
    $("#conteudo_geral").load(
      "menu_pedidos/telas/novo_pedido_mobile.php",
      function () {
...
function ve_compartilhamento_mesa() {
  $("#exibe_compartilhamento_mesa_recebimento").html(
    "<br><br><CENTER><h4>CARREGANDO...</h4><br><br></CENTER>"
  );
  $("#ModalCompartilhamentoMesaRecebimento").modal();
  $("#exibe_compartilhamento_mesa_recebimento").load(
    "menu_pedidos/listagem/lista_compartilhamento_mesa.php"
  );
}

function modal_edita_cliente_pedido(cliente) {
  $("#exibe_dados_cliente_pedido").html(
    "<br><CENTER><h4>CARREGANDO...</h4></CENTER><br>"
  );
  $("#ModalEditaDadosClienteVenda").modal();

  $.post(
    "menu_clientes/telas/tela_edicao_ficha_cliente.php",
    { id: cliente },
    function (resposta) {
...
function atualiza_mesas_automatico() {
  carregando();
  $("#conteudo_geral").load(
    "menu_pedidos/telas/mesas_pedidos.php",
    function () {
      vendas_internet();
    }
  );
...
function atualiza_entregas_automatico() {
  carregando();
  $("#conteudo_geral").load(
    "menu_pedidos/telas/entregas_comandas.php",
    function () {
      vendas_internet();
    }
  );
...
function cancela_ficha() {
  ...
  $.post(
    "menu_pedidos/actions/cancela_venda.php",
    { id: 1 },
    function (resposta) {}
  );
}
...
function busca_cliente_nome() {
  ...
  $.post(
    "menu_clientes/listagem/listagem_clientes.php",
    { nome: nome },
    function (resposta) {
      $("#btn_buscar2").html(
...
function busca_cliente(a, modulo_pontos = 0) {
  ...
  $.post(
    "ajax/procura_cliente.php",
    { telefone: telefone, cartao: cartao, tipo: a },
    function (resposta) {
      $("#btn_buscar").html(
        '<i class="icofont-search-user" style="font-size: 17px"></i>'
      );
      ...
      if (resposta == 0) {
        ...
        if ($("#outros_enderecos").length) {
          if (val[13] != "") {
            ...
            $("#outros_enderecos").html(
              "<option>NENHUM OUTRO ENDEREÇO CADASTRADO</option>"
            );
          }
        }
        ...
        if (modulo_pontos == 1) {
          ...
          $("#pontos_resgatar").val("------");
          $.post(
            "includes/verifica_pontuacao_cliente.php",
            { id_cliente_venda: val[0], exibe_pontos: 1 },
            function (resposta) {
...
function inicia_pedido(mesa = 0, mobile = 0) {
  carregando();
  sim_novo_pedido = 0;
  $("#ModalIniciaNovoPedido").modal("hide");

  $.post(
    "menu_caixa/actions/verifica_sistema.php",
    { id: 1 },
    function (resposta) {
      var val = resposta.split("&@&");
      ...
      if (val[0] == 1) {
        ...
        $.post(
          "menu_pedidos/actions/inicia_pedido.php",
          {
            mesa: mesa,
        ...
        if (mobile == 1) {
          ...
          $("#conteudo_geral").load(
            "menu_caixa/telas/abre_caixa.php",
            function () {
              $("#troco").focus();
            }
          );
...
function seleciona_produtos(
  ...
  if (pdv == 0) {
    ...
    if (global_pesquisa_adicionais == 0 || global_produto_selecionado != id) {
      $("#exibicao_adicionais_produto_selecionado").html(
        "<center><br><br>CARREGANDO...</center>"
      );
      $("#exibicao_adicionais_produto_selecionado").load(
        "menu_pedidos/listagem/listagem_adicionais.php?categoria=" + categoria
      );
    ...
    if (global_pesquisa_opcoes == 0 || global_produto_selecionado != id) {
      $("#exibicao_opcoes_produto_selecionado").html(
        "<center><br><br>CARREGANDO...</center>"
      );
      $("#exibicao_opcoes_produto_selecionado").load(
        "menu_pedidos/listagem/listagem_opcoes.php?categoria=" +
          categoria +
          "&produto=" +
          id
      );
  ...
  if (tipo == 1) {
    ...
    if (qtd > 0) {
      ...
      if (preco_composto == 1) {
        ...
        $.post(
          "menu_pedidos/listagem/listagem_variacao_tamanhos.php",
          { categoria: categoria, produto: id },
          function (resposta) {
...
function exclui_opcao_pedido(id) {
  ...
  if (pedido_aguarda_venda == 1) {
    ...
    $.post(
      "menu_pedidos/actions/exclui_opcao_venda.php",
      { id: id },
      function (resposta) {
        $("#resumo-pedido-comanda").load(
          "menu_pedidos/listagem/listagem_itens_pedido.php"
...
function exclui_adicional_pedido(id) {
  ...
  if (pedido_aguarda_venda == 1) {
    ...
    $("#resumo-pedido-comanda").html("<br><center>CARREGANDO...</center>");
    $.post(
      "menu_pedidos/actions/exclui_adicional_venda.php",
      { id: id },
      function (resposta) {
        $("#resumo-pedido-comanda").load(
          "menu_pedidos/listagem/listagem_itens_pedido.php"
...
function exlcui_produto_pedido(tipo, id) {
  ...
  if (pedido_aguarda_venda == 1) {
    ...
    $("#resumo-pedido-comanda").html("<br><center>CARREGANDO...</center>");
    $.post(
      "menu_pedidos/actions/exclui_produto_venda.php",
      {
        id_prod_venda_cancelamento: id,
        pedido_aguarda_venda: pedido_aguarda_venda,
      },
      function (resposta) {
        var tela_mobile = $("#tela-mobile").val();
...
function fazdesconto(tipo) {
  ...
  if (tipo == 1) {
    ...
    var valor_percente =
      (parseFloat(total_pedido) * parseFloat(desconto)) / 100;
    var desconto_final =
      parseFloat(total_pedido) -
      parseFloat(valor_percente) +
      parseFloat(total_entrega);
  ...
  if (tipo == 3) {
    ...
    //% GARCOM//
    var fazx =
      (parseFloat(desconto_final) * parseFloat(porcentagem_garcom)) / 100;
    $("#soma_garcom").val(fazx);
    $("#val_final_garcom").html("" + fazx.toFixed(2));
...
function ve_pagamentos_recebidos() {
  $("#exibe_pagamentos_recebidos").html(
    "<br><br><CENTER><h4>CARREGANDO...</h4><br><br></CENTER>"
  );
  $("#ModalPgtosRecebidos").modal();
  $("#exibe_pagamentos_recebidos").load(
    "menu_pedidos/listagem/lista_pagamentos_venda.php"
  );
}
...
function altera_nome_cliente_comanda(campo) {
  ...
  $.post("menu_pedidos/actions/altera_nome_divisao_mesa_comanda.php", {
    id: campo,
    nome: nome,
  });
  var_global = 0;
...
function altera_quantidade_produto(campo) {
  ...
  $.post(
    "menu_pedidos/actions/altera_quantidade.php",
    { id: campo, qtd: qtd },
    function (a) {
      atualiza_valor_final_pedido_comanda();
    }
...
function altera_valor_produto(campo) {
  ...
  $.post(
    "menu_pedidos/actions/altera_valor_produto.php",
    { id: campo, valor: valor },
    function (a) {
      atualiza_valor_final_pedido_comanda();
    }
  );
...
function atualiza_valor_final_pedido_comanda() {
  $.post(
    "menu_pedidos/actions/salva_totais_pedido.php",
    { id: 1 },
    function (resposta) {
...
function pesquisa_produtos_venda(pesquisa) {
  $("#categoria_selecionada_mobile").val(
    $("#categoria_selecionada_mobile option:eq(0)").val()
  );
...
function salva_item_pedido() {
  ...
  $.post(
    "menu_pedidos/actions/salva_produto_venda.php?meio_meio=" +
      meio_meio +
      "&opcionais=" +
      camposMarcados +
      "&opcoes_produto=" +
    ...
    function (resposta) {
      ...
      if (venda_aguarde != 0) {
        ...
        if (
          tipo_impressao_item_avulso == "" ||
          tipo_impressao_item_avulso == "ITEM A ITEM"
        ) {
...
function apenas_salva_pedido() {
  ...
  if (venda_aguarde != 0) {
    var tipo_impressao_item_avulso = $("#impressao_item_avulso").val();
    if (
      tipo_impressao_item_avulso == "JUNTO APENAS UMA VEZ" &&
...
function finaliza_pedido2(imprime = 0, reload = 0) {
  ...
  if (tela_mobile == 1) {
    ...
    if (entrega != 0) {
      if (
        nome_cliente_mobile == "" ||
        nome_cliente_mobile == "CLIENTE AVULSO"
      ) {
  ...
  $.post(
    "menu_pedidos/actions/salva_pedido_final.php",
    {
      remove_taxa: remove_taxa,
      soma_garcom: soma_garcom,
      nome_cliente_mobile: nome_cliente_mobile,
      embala_viagem: embala_viagem,
      pre_tipo_pagamento: pre_tipo_pagamento,
      levar_maquina_cartao: levar_maquina_cartao,
      entregador: entregador,
      troco_leva_maquina: troco_leva_maquina,
...
function realiza_pagamento() {
  ...
  if ($("#venda_fiscal").length) {
    if ($("#venda_fiscal").is(":checked")) {
      ...
      if (valor_recebe < final_venda) {
        exibe_erros_gerais(
          "Não é possível receber menos que o total da venda."
        );
        $("#icon-ok-val-recebe").hide();
        $("#icon-erro-val-recebe").show();
        return;
  ...
  if (forma_pagamento == 3) {
    if (venda_fiscal == 1) {
      exibe_erros_gerais(
        "Não é possível gerar um Cupom Fiscal para essa forma de pagamento."
      );
      return;
    }
  ...
  $.post(
    ...
    function (resposta2) {
      ...
      $.post(
        "menu_pedidos/actions/salva_recebimento.php",
        {
          venda_fiscal: venda_fiscal,
          pontos_validos_troca: pontos_validos_troca,
          utiliza_resgate_pontos: utiliza_resgate_pontos,
          troco_recebe: troco_recebe,
          forma_pagamento: forma_pagamento,
          valor_recebe: valor_recebe,
        },
        function (resposta) {
          var val = resposta.split("&@&");
...
function apenas_encerra_venda() {
  $.post(
    "menu_pedidos/actions/finaliza_venda.php",
    { finaliza: 1 },
    function (resposta) {
      inicia_sistema();
    }
...
function resgate_pontos() {
  ...
  $.post(
    "menu_pedidos/actions/calcula_desconto_pontuacao.php",
    { final: pontos_validos },
    function (resposta) {
